...
"""
Deprecated in favor of create_geopackage_index / style_geopackage_index.
...
available_campaigns = {
    ...
    "NASA_2017_ICEBRIDGE_AIR_BM3.csv": (
        "CRESIS/2017_Antarctica_P3",
        "CRESIS/2017_Antarctica_Basler",
    ),
...
def add_bedmap_layers(root_group, data_dir):
    ...
    for institution in institutions:
        ...
        for filename in filenames:
            ...
            # All bedmap lines that are plotted at this step are unavailable.
...
def add_spri_layers(root_group, spri_dir):
    ...
    if len(stanford_index) == 0:
        print(
            "Could not find STANFORD group in root's children: {}".format(
                root_group.children()
            )
        )
    ...
    for flight in flights:
        ...
        # All SPRI data is technically available, but unsupported because
        # it's still only in scanned images.
...
def add_kml_campaigns(region: str, provider: str, root_group, index_dir: str):
    ...
    if len(provider_index) == 0:
        print(
            "Could not find {} group in root's children: {}".format(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "spri_directory", help="directory where the SPRI csv files live. "
    )
